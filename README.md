...
This repository also includes a small **UVM** testbench under `uvm/`. For the first public drop, the flow that is expected to work end-to-end is **AMD/Xilinx Vivado XSIM** (xvlog / xelab / xsim) using the simulator-bundled UVM library (`-L uvm`). Other simulators may work, but they are not part of the supported matrix for this release.

**Prerequisites**

- Vivado on `PATH` (for example `source .../Vivado/<version>/settings64.sh`), or set `VIVADO_ROOT` to `.../Vivado/<version>/Vivado` so `scripts/uvm_vivado_xsim.sh` can source `settings64.sh`.
- [`sv2v`](https://github.com/zachjs/sv2v) on `PATH` (optional: place the binary in `tools/bin/`; the script prepends that directory to `PATH`).
- GNU `make` (the script runs `make compile` to emit `build/gpu.v` from `src/*.sv`, same as the cocotb flow).

**Run**

```bash
./scripts/uvm_vivado_xsim.sh              # default UVM test: gpu_matadd_test
./scripts/uvm_vivado_xsim.sh gpu_matadd_test
```

Artifacts are written under `sim_build/vivado_uvm/` (ignored by git). To compile and elaborate only, without launching the simulator:

```bash
UVM_VIVADO_COMPILE_ONLY=1 ./scripts/uvm_vivado_xsim.sh
```
